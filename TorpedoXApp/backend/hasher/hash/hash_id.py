...
def SHA512(hash):
    ...
    #hs='afb9e3c77a678e33f31aa716f09df69c03bbfae8c14d286869f4d9f6b806f8f8c6702f8db8f4f4e35ce98b13576261434b93dfbc61f49fcac014f1b68dcfedd2'
    if len(hash)==len(hs) and hash.isdigit()==False and hash.isalpha()==False and hash.isalnum()==True:
        jerar.append("122020")
def SHA512HMAC(hash):
...
def remove_spaces(hash):
    return hash.replace(" ", "")

def analyze(cipher_text):
    jerar = []
    h = remove_spaces(cipher_text)
    ...
    if len(jerar) == 0:
        ...
        jerar.sort()
        return f"Possible Hashs: {algorithms[jerar[0]]}, {algorithms[jerar[1]]}"
    else:
        jerar.sort()
        return f"Possible Hashs: {', '.join([algorithms[j] for j in jerar])}"
...
while True:
    try:
        ...
        h = remove_spaces(h)  # Remove spaces from the input
